...
import { PiggyBank, PlusCircle, Wallet, CalendarDays, X } from "lucide-react";
...
function Savings() {
  ...
  const { savings = [], income = [], expenses = [], setSavings, loading } = useData();
  ...
  const handleSaveSaving = async () => {
    try {
      const response = await fetch("https://finance-api-2.onrender.com/savings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...newSaving, amount: Number(newSaving.amount) }),
      });
      ...
      alert("Failed to add savings. Check console for details.");
    }
  };
    ...
    <div className={`p-6 space-y-6 transition-colors duration-150 ease-in-out ${darkMode ? "bg-neutral-900 text-gray-100" : "bg-gray-100 text-gray-900"}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`p-4 rounded-xl border shadow-md ${darkMode ? "bg-neutral-900 border-neutral-700" : "bg-white border-gray-300"}`}>
          <h3 className="text-sm text-neutral-500">Total Savings</h3>
          <p className="text-xl font-bold mt-1 text-green-600">₹ {totalSavings.toLocaleString()}</p>
        </div>
        <div className={`p-4 rounded-xl border shadow-md ${darkMode ? "bg-neutral-900 border-neutral-700" : "bg-white border-gray-300"}`}>
        ...
        <div className={`p-4 rounded-xl border shadow-md ${darkMode ? "bg-neutral-900 border-neutral-700" : "bg-white border-gray-300"}`}>
          ...
          <p className="text-xl font-bold mt-1">{filteredSavings.length}</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl border shadow-md p-6 transition-colors duration-150 ease-in-out ${darkMode ? "bg-neutral-900 border-neutral-700" : "bg-white border-gray-300"}`}>
        <h2 className="font-semibold text-lg mb-4">Recent Savings</h2>
